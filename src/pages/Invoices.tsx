  ...
  export function Invoices() {
    ...
    useEffect(() => {
      ...
      api.get(`/invoices/years?client_id=${selectedClient}`).then((res) => {
        ...
        uniqueYears = Array.from(new Set(years)).sort((a, b) => Number(b) - Number(a));
    ...
    const handleUpload = async () => {
      ...
      try {
        ...
        const uploadedClientId = uploadedInvoice.client.id.toString();
    
        // Se a fatura enviada é do cliente que está selecionado, apenas adiciona ela na lista
        if (uploadedClientId === selectedClient) {
          setInvoices((prev) => [...prev, uploadedInvoice]);
        } else {
          // Troca o filtro para o cliente correto e busca as faturas dele
          ...
          <div style={styles.filterItem}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const yearOnly = selected.split('/')[1] || selected;
                setSelectedYear(yearOnly);
              }}
              disabled={!availableYears.length}
            >
              <option value="">Todos</option>
              {availableYears.map((ref) => {
                const year = ref.split('/')[1] || ref;
                return (
